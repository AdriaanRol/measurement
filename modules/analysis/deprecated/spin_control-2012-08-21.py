...
def plot_data(datapath, fit_data = True, title='',with_detuning = False, save = True):
...
def plot_SE(datapath, fid=(0.8045,0.9906),fiderr=(0.01,0.01),fit_data = True, exp_guess=2.8, nr_of_pulses=1,save = True):
    ...
    if 'fe_min' in e.keys():
        ...
        if 'free_evol' in e.keys():
            ...
            tau_free_evol = e['free_evol']*1e-6
        else:
    ...
    plt.xlabel('Free evolution time [ms]')
    plt.ylabel('Integrated counts')
