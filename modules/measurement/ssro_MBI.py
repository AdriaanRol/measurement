...
# load dictionary of sil
        ...
        self.MW_freq = self.sildic['MW_freq_center'] - self.sildic['hf_splitting']
    ...
    def generate_MW_sweep_sequence(self,do_program=True, lt1 = False):
        ...
        for i in np.arange(self.nr_of_datapoints):
            MBIseq.MBI_element(seq,el_name='MBI_pulse'+str(i), 
                   jump_target='spin_control_'+str(i)+'RO_step_0',
                   goto_target='MBI_pulse'+str(i))
            for k in np.arange(self.nr_of_RO_steps):
                el_name = 'spin_control_'+str(i)+'RO_step_'+str(k)
                if (i == self.nr_of_datapoints-1) and (k==self.nr_of_RO_steps-1):
                ...
                for j in np.arange(self.nr_of_MW_pulses):
                    ...
                    seq.add_pulse('MW_pulse'+str(j),channel=chan_mwI,element=el_name,start=0,
                    ...
                    seq.add_pulse('MW_pulse_mod'+str(j),channel=chan_mw_pm,element=el_name,
                        ...
                        start_reference='MW_pulse'+str(j),link_start_to='start',
                        duration_reference='MW_pulse'+str(j),link_duration_to='duration', amplitude=2.0)
    ...
    def generate_RF_sweep_sequence(self,do_program=True, lt1 = False):
        seq = Sequence('spin_control')
        ...
        for i in np.arange(self.nr_of_datapoints):
            ...
            last = MBIseq.shelving_pulse(seq, pulse_name = 'shelving_pulse_'+str(i),
            ...
            seq.add_pulse('wait_before_readout', channel = chan_mwI, element = 'spin_control'+str(i),
                    ...
                    link_start_to = 'end', shape = 'rectangular')
            print "Added RF pulse of amplitude "+str(self.RF_pulse_amp[i])
            

            MBIseq.readout_pulse (seq, pulse_name = 'readout_pulse_'+str(i),
                   el_name = 'spin_control'+str(i), first = 'wait_before_readout')          

            
        self.par['sequence_wait_time'] = (2*self.MBIdic['wait_time_before_MBI_pulse']+
                                        self.MBIdic['MBI_pulse_len']+
                                        self.RF_pulse_len.max()+
                                        self.MBIdic['wait_time_before_shelv_pulse']+
                                        self.pulsedic['shelving_len']+3000)/1000
    ...
    def spin_control(self, data, lt1=False):
        ...
        self.par['Ex_RO_voltage'] = self.ins_E_aom.power_to_voltage(self.ssrodic['Ex_RO_amplitude'])
        self.par['A_RO_voltage'] = self.ins_A_aom.power_to_voltage(self.ssrodic['A_RO_amplitude'])
        ...
        self.adwin.start_MBI_Multiple_RO(
            ...
            RO_duration = self.ssrodic['RO_duration'],
        ...
        SSRO_data   = self.physical_adwin.Get_Data_Long(27,1,
                        sweep_length * self.nr_of_RO_steps)
...
def sweep_MW_amp (lt1 = False, name = 'SIL9_lt2_sweep_MW_amp', min_amp = 0.6, max_amp = 1.2, MW_len=140,nr_of_MW_pulses=1,nr_of_datapoints = 21,reps=500,RO_reps=1):
    datafolder= 'D:/measuring/data/'
    date = dtime.strftime('%Y') + dtime.strftime('%m') + dtime.strftime('%d')
    datapath = datafolder+date + '/'
...
def sweep_MW_len (lt1 = False, name = 'SIL9_lt2_sweep_MW_len', min_len = 10, max_len = 200, MW_amp=0.5,nr_of_MW_pulses=1,nr_of_datapoints = 21,reps=500,RO_reps=1):
    datafolder= 'D:/measuring/data/'
    date = dtime.strftime('%Y') + dtime.strftime('%m') + dtime.strftime('%d')
    datapath = datafolder+date + '/'
...
def sweep_RF_amp (lt1 = False, name = 'SIL9_lt2_sweep_RF_amp', min_amp = 0., max_amp = 0.200, RF_len=0.5,nr_of_datapoints = 21,reps=500):
    datafolder= 'D:/measuring/data/'
    date = dtime.strftime('%Y') + dtime.strftime('%m') + dtime.strftime('%d')
    datapath = datafolder+date + '/'
...
def sweep_RF_len (lt1 = False, name = 'SIL9_lt2_sweep_RF_len', min_len = 0., max_len = 50000, RF_amp=0.1,nr_of_datapoints = 21,reps=500):
    datafolder= 'D:/measuring/data/'
    date = dtime.strftime('%Y') + dtime.strftime('%m') + dtime.strftime('%d')
    datapath = datafolder+date + '/'
...
def nmr (lt1 = False, name = 'SIL9_lt2_nmr', min_freq = 2.743e6, max_freq = 2.78e6, nr_of_datapoints = 21,reps=500):
    datafolder= 'D:/measuring/data/'
    date = dtime.strftime('%Y') + dtime.strftime('%m') + dtime.strftime('%d')
    datapath = datafolder+date + '/'
...
### Old measurement functions
'''
def nmr (lt1 = False, name = 'SIL9_lt2_NMR', min_freq = 2.743e6, max_freq = 2.78e6, nr_of_datapoints = 21):
    datafolder= 'D:/measuring/data/'
    date = dtime.strftime('%Y') + dtime.strftime('%m') + dtime.strftime('%d')
    datapath = datafolder+date + '/'
    ...
    m.RF_freq=np.linspace(min_freq,max_freq,nr_of_datapoints)
    m.nr_of_datapoints = nr_of_datapoints
    m.par['sweep_par'] = m.RF_freq
    m.par['sweep_par_name'] = 'RF freq (KHz)'

    m.start_measurement (m.generate_sequence)
    dp = get_datapath()
    path = lde_calibration.find_newest_data (dp, string=name)
    spin_control.plot_data_MBI(path)
...
def RF_rabi (lt1 = False, name = 'SIL9_lt2_RF_rabi', min_pulse_len = 0., max_pulse_len = 60000, nr_of_datapoints = 11):
    datafolder= 'D:/measuring/data/'
    date = dtime.strftime('%Y') + dtime.strftime('%m') + dtime.strftime('%d')
    datapath = datafolder+date + '/'
    ...
    m.RF_dur=np.linspace(min_pulse_len,max_pulse_len,nr_of_datapoints)
    m.nr_of_datapoints = nr_of_datapoints
    m.par['sweep_par']= m.RF_dur
    print "datapoints: "+str(m.par['sweep_par'])
    m.par['sweep_par_name'] = 'RF duration (ns)'
...
def RF_amp (lt1 = False, name = 'SIL9_lt2_RF_amp', min_amp = 0, max_amp = .2, nr_of_datapoints = 21):
    datafolder= 'D:/measuring/data/'
    date = dtime.strftime('%Y') + dtime.strftime('%m') + dtime.strftime('%d')
    datapath = datafolder+date + '/'
    ...
    m.RF_amp=np.linspace(min_amp,max_amp,nr_of_datapoints)
    m.nr_of_datapoints = nr_of_datapoints
    m.par['sweep_par'] = m.RF_amp
    m.par['sweep_par_name'] = 'RF amplitude (V)'
    ...
    path = lde_calibration.find_newest_data (dp, string=name)
    spin_control.plot_data_MBI(path)
def nuclear_ramsey (lt1 = False, name='SIL9_lt2_nuclear_ramsey_10_15ms', min_tau = 0., max_tau = 2., nr_of_datapoints = 11):
    
    #tau is expressed in milliseconds (for the parameters passed to the script)
    datafolder= 'D:/measuring/data/'
    date = dtime.strftime('%Y') + dtime.strftime('%m') + dtime.strftime('%d')
    datapath = datafolder+date + '/'
    ...
    m.par['min_tau'] = min_tau*1e6
    m.par['max_tau'] = max_tau*1e6
    m.nr_of_datapoints = nr_of_datapoints
    tau = np.linspace(min_tau, max_tau, nr_of_datapoints)
    m.par['sweep_par'] = tau
    m.par['sweep_par_name'] = 'free evolution time [ms]'
    
    m.start_measurement(m.generate_sequence_Ramsey)
    dp = get_datapath()
    path = lde_calibration.find_newest_data (dp, string=name)
    spin_control.plot_data_MBI(path)
...
#        self.par['sequence_wait_time'] = self.sequence_wait_time
 ...
 def generate_sequence_Ramsey (self, do_program = True, lt1=False):
    ...
    # decoupling curve)
        ...
        self.par['sequence_wait_time'] = max_seq_time_us
