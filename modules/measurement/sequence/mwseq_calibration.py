...
def Pi_Pulse_amp(sweep_param,pulse_dict,lt1 = False,do_program=True):
    ...
    duty_cycle_time = pulse_dict["duty_cycle_time"]
    axis=pulse_dict["pulse_axis"]
    freq=pulse_dict["freq"]
    shape = pulse_dict["shape"]
    doenv = pulse_dict["do_env"]
    if axis =='X':
        chan=chan_mwI
    else:
        chan=chan_mwQ
    for i in np.arange(nr_of_datapoints):
            ...
            if j == 0:
                if istate_pulse["Do_Pulse"]: 
                    seq.add_pulse('first_wait', channel = chan_mw_pm, 
                        ...
                        duration = 50, amplitude = 0)
                    if doenv:
                        seq.add_pulse('init_state_pulse' + str(j), channel = chan, element = 'spin_control_'+str(i+1),
                            start_reference = 'first_wait', link_start_to = 'end', start = 0, 
                            duration = istate_pulse["duration"], amplitude = istate_pulse["amplitude"], shape = shape,frequency=freq,envelope='erf')
                    else:
                        seq.add_pulse('init_state_pulse' + str(j), channel = chan, element = 'spin_control_'+str(i+1),
                            start_reference = 'first_wait', link_start_to = 'end', start = 0, 
                            duration = istate_pulse["duration"], amplitude = istate_pulse["amplitude"], shape = shape,frequency=freq)
                    ...
                    seq.add_pulse('wait' + str(j), channel = chan, element = 'spin_control_'+str(i+1), 
                        start_reference = 'init_state_pulse' + str(j), link_start_to='end',start = 0, 
                        duration = time_between_pulses, amplitude = 0)
                else:
                    seq.add_pulse('wait' + str(j), channel = chan_mw_pm, 
                ...
                seq.add_pulse('wait' + str(j), channel = chan, 
                        element = 'spin_control_'+str(i+1), start = 0, 
                        start_reference=last, link_start_to='end',
                        duration = time_between_pulses, amplitude = 0)
            if doenv:
                ...
                seq.add_pulse('pi' + str(j), channel = chan, element = 'spin_control_'+str(i+1),
                    start = 0, start_reference = 'wait'+str(j), link_start_to = 'end',
                    duration = pi["duration"], amplitude = amplitude[i], shape = shape,frequency=freq,envelope='erf')
            else:
                seq.add_pulse('pi' + str(j), channel = chan, element = 'spin_control_'+str(i+1),
                    start = 0, start_reference = 'wait'+str(j), link_start_to = 'end',
                    duration = pi["duration"], amplitude = amplitude[i], shape = shape,frequency=freq)
        ...
        seq.add_pulse('final_wait', channel = chan, element = 'spin_control_'+str(i+1),
...
def Multiple_DSC_Pulses(sweep_param,pulse_dict,lt1 = False,do_program=True):
    ...
    if lt1:
        chan_mwI = 'MW_Imod_lt1'
        chan_mwQ = 'MW_Qmod_lt1'
        chan_mw_pm = 'MW_pulsemod_lt1' #is connected to ch1m1
    else:
        chan_mwI = 'MW_Imod'
        chan_mwQ = 'MW_Qmod'
        chan_mw_pm = 'MW_pulsemod' #is connected to ch1m1
...
def SE_regular(sweep_param,pulse_dict,lt1 = False,do_program=True):
    '''
    SE with regular pulses, sweep second tau.
    
    
    sweep_param = numpy array with tau2 durations
    pulse_dict={
                "Pi":{"duration": ...},
                "Pi_over_2:{"duration":...}
    ...
    pi = pulse_dict["Pi"]
    pi_over_2 = pulse_dict["Pi_over_2"]    
    tau1 =  pulse_dict["tau1"]
    duty_cycle_time = pulse_dict["duty_cycle_time"]
    ...
    for i in np.arange(nr_of_datapoints):
        ...
        seq.add_pulse('first_pi_over_2', channel = chan_mwI, element = 'spin_control_'+str(i+1),
                    start = 0, duration = pi_over_2["duration"], amplitude = pi_over_2["amplitude"], start_reference = 'first_wait',
                    link_start_to = 'end', shape = 'rectangular')

        seq.add_pulse('pm_first_pi_over_2', channel = chan_mw_pm, element = 'spin_control_'+str(i+1),
                start=-MW_pulse_mod_risetime, duration=2*MW_pulse_mod_risetime, 
        ...
        seq.add_pulse('tau_1', channel = chan_mw_pm, 
                element = 'spin_control_'+str(i+1),start = 0,
                start_reference= 'first_pi_over_2',link_start_to = 'end', 
                duration = tau1, amplitude = 0)
        ## pi pulse
        seq.add_pulse('pi', channel = chan_mwI, element = 'spin_control_'+str(i+1),
                    start = 0, duration = pi["duration"], amplitude = pi["amplitude"], start_reference = 'tau_1',
                    link_start_to = 'end', shape = 'rectangular')

        seq.add_pulse('pm_pi', channel = chan_mw_pm, element = 'spin_control_'+str(i+1),
                start=-MW_pulse_mod_risetime, duration=2*MW_pulse_mod_risetime, 
        ...
        seq.add_pulse('tau_2', channel = chan_mw_pm, 
                element = 'spin_control_'+str(i+1),start = 0,
                start_reference= 'pi'  ,link_start_to = 'end', 
                duration = sweep_param[i], amplitude = 0)
        
        #second Pi over 2
        seq.add_pulse('second_pi_over_2', channel = chan_mwI, element = 'spin_control_'+str(i+1),
                    start = 0, duration = pi_over_2["duration"], amplitude = pi_over_2["amplitude"], start_reference = 'tau_2',
                    link_start_to = 'end', shape = 'rectangular')

        seq.add_pulse('pm_second_pi_over_2', channel = chan_mw_pm, element = 'spin_control_'+str(i+1),
                start=-MW_pulse_mod_risetime, duration=2*MW_pulse_mod_risetime, 
        ...
        seq.add_pulse('final_wait', channel = chan_mw_pm, element = 'spin_control_'+str(i+1),
                start_reference = 'second_pi_over_2',link_start_to ='end', duration = duty_cycle_time, amplitude = 0) 
