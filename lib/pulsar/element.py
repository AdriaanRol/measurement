...
class Element:
    ...
    def offset(self):
        ...
        for p in self._pulses:
            for c in self._pulses[p].channels:
                t0s.append(self._pulses[p].t0() - self._channels[c]['delay'])
    ...
    def _auto_pulse_name(self, base='pulse'):
        i = 0
        while base+'-'+str(i) in self._pulses:
            i += 1
        return base+'-'+str(i)

    def add(self, pulse, name=None, t0=0, 
        ...
        if refpulse != None:
            ...
            t0 += self._pulses[refpulse].end()

            if refpoint == 'start':
                t0 -= self._pulses[refpulse].length
            elif refpoint == 'center':
                t0 -= self._pulses[refpulse].length/2.
    ...
    def pulse_start_time(self, pname, cname):
        return self._pulses[pname].t0() - self._channels[cname]['delay'] - \
            self.offset()

    def pulse_end_time(self, pname, cname):
        return self._pulses[pname].end() - self._channels[cname]['delay'] - \
            self.offset()

    def pulse_length(self, pname):
        return self._pulses[pname].length
    ...
    def pulse_samples(self, pname):
        return self._time2sample(self._pulses[pname].length)
    ...
    def ideal_waveforms(self):
        ...
        for p in self._pulses:
            ...
            pulsewfs = self._pulses[p].wf(tvals[:psamples])
            for c in self._pulses[p].channels:
                if not c in wfs:
