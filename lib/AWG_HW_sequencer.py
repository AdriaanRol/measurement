...
###############################################################################
    ...
    def _add_pulse(self, element, offset, pulse, waveforms):
        ...
        duration = (eidx-sidx)/self.clock

        ### rectangular pulses
        if pulse['shape'] == 'rectangular':
            amplitudes = self._rect_pulse(pulse, chan, eidx-sidx)
        ...
        ### starting time within the element
            ...
            if AWG_subchan_idx == 0:
                amplitudes = self._cosine_pulse(pulse, chan, eidx-sidx, stime)
        ...
        ### waveforms for two channels, both of which are required
            ...
            if len(qpulses) > 0:
                ...
                iamp, qamp = self._iqmod_pulse(ipulse, (chan, qchan), eidx-sidx)
        ...
        # do nothing here, we only react on I.
        elif pulse['shape'] == 'IQmod-Q':
            return waveforms
    ...
    # rectangular pulse
    def _rect_pulse(self, pulse, chan, samples):
        pts = ones(samples)*pulse['amplitude']
        return self._voltage_to_waveform(chan, pts)

    # cosine pulse
    def _cosine_pulse(self, pulse, chan, samples, t0):
        frq = pulse['frequency']
        amplitude = pulse['amplitude']
        phase = pulse['phase']

        t = linspace(t0, t0 + samples/self.clock, samples)
        cosine = self._voltage_to_waveform(chan, amplitude * \
                cos(2*pi*(frq*t + phase/360.)))

        return cosine

    def _iqmod_pulse(self, ipulse, (ichan, qchan), samples):
        frq = ipulse['frequency']
        amplitude = ipulse['amplitude']
        phase = ipulse['phase']

        shaper = PulseShaper(self.clock, samples)
        i,q = shaper.IQmod_wform([frq], [1], [phase])

        iscaled = self._voltage_to_waveform(ichan, i*amplitude)
        qscaled = self._voltage_to_waveform(qchan, q*amplitude)

        return iscaled, qscaled
...
class PulseShaper():
    ...
    def IQmod_wform(self, frqs, amplitudes, phases):
        ...
        t = np.linspace(0, self.samples/self.clock, self.samples)
        I_wform = np.zeros(len(t))
        Q_wform = np.zeros(len(t))

        for i,f in enumerate(frqs):
            I_wform += amplitudes[i] * np.cos(2*np.pi*(t*f+phases[i]/360.))
            Q_wform += amplitudes[i] * np.sin(2*np.pi*(t*f+phases[i]/360.))
