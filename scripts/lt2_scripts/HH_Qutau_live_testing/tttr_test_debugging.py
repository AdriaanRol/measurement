...
def just_read_out_qutau(meastime, BuffSize):
    ...
    stopmaar = False
    save = False
    sp = r'D:\measuring\data\20130705'
    ...
    #print "Start saving data to disk..."
    #np.savez(os.path.join(sp, tm, 'data.npz'), ch = ch_dump, ts = ts_dump)
    print "Done!"
...
def analyze_dts(ts_dump, errs):
    ...
    for k in range(len(ts_dump)):
        ...
        dts = errors.analyze_timestamps(ts_dump[str(k)].astype(np.uintc), 
                np.asarray(errs[k]).astype(np.uintc))
