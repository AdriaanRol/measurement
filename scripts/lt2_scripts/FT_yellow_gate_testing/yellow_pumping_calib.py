...
class SpinPumping(Measurement):
    ...
    def setup(self,lt1=False):
        ...
        if lt1:
            ...
            self.ins_Y_aom=qt.instruments['YellowAOM']
    ...
    def start_measurement(self,generate_sequence,sp_amp,lt1=False,ssro_dict={}):
        ...
        self.par['SSRO_repetitions'] =               int(len(self.par['sweep_par'])*self.par['reps_per_datapoint'])
    ...
    def spin_control(self,lt1,ssro_dict={}):
        ...
        self.par['green_off_voltage'] = 0.0#self.ins_green_aom.power_to_voltage(self.par['green_off_amplitude'])
        ...
        self.adwin.start_singleshot(load=True,
        ...
        ro_time = np.arange(self.par['SSRO_duration'])*self.par['cycle_duration']*3.333
...
def main(lt1=False, name='yellowpumping',p_power=5e-9, ssro_dict={}):
    name=name+'_'+str(p_power)

    m = SpinPumping(name,p_power=p_power)
    m.setup(lt1)

    
    
    nr_of_datapoints=1
    sp_min_duration=30000 #ns
    sp_max_duration=30000 #ns
    

    m.ins_Y_aom.apply_voltage(0)
    m.ins_Y_aom.set_cur_controller('AWG')
    yellow_amp=m.ins_Y_aom.power_to_voltage(p_power)
    m.ins_Y_aom.set_cur_controller('ADWIN')
    ...
    m.yellow_threshold = 1
    m.short_CR_duration = 30
    m.physical_adwin.Set_Par(74,m.yellow_threshold)
    m.physical_adwin.Set_Par(77,m.short_CR_duration)
    m.physical_adwin.Set_Par(78,m.par['CR_duration'])
    
    m.start_measurement(SP_seq,yellow_amp,lt1=lt1,ssro_dict=ssro_dict)
