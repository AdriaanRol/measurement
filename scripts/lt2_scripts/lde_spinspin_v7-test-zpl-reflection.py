...
long_pulse_settings = False # longer eom pulse settings
mw_on = False # MW on (SMB status set to on before measurement), in debug_mode MW is always off
...
class LDEMeasurement(meas.Measurement):
    ...
    def generate_sequence(self, do_program=True):
        ...
        seq.add_pulse('pi2_pulse_mod_lt1', channel = chan_mw_pm_lt1, element = 'lde',
                start=-self.MW_pulsemod_risetime_lt1, duration=2*self.MW_pulsemod_risetime_lt1, 
                start_reference = 'pi2_pulse_lt1', link_start_to = 'start', 
                duration_reference = 'pi2_pulse_lt1', link_duration_to = 'duration', 
    ...
    def generate_sequence_regular_pulses(self, do_program=True): #######FIXME FIXME only the other sequence has been adopted for 2 pulse mod
        awg.set_event_jump_timing('SYNC')
        self.seq = Sequence('lde')
        seq = self.seq
...
#NOTE debug zpl pulse
#        seq.add_pulse('EOM_pulse_debug'+str(i),  chan_eom, 'lde', 
#                amplitude = self.eom_pulse_amplitude - self.eom_off_amplitude,
#                start = 0, duration = 10, 
#                start_reference = 'EOM_pulse'+str(i), link_start_to = 'end')
  ...
  #              start_reference = 'EOM_pulse'+str(i), link_start_to = 'end')
        ...
        # 5c: add PLU gate
        seq.add_pulse('plu-gate'+str(i), chan_plusync, 'lde',
                start = self.plu_2_start_offset+2,
                duration = self.plu_gate_duration,
                amplitude = 2.0,
                start_reference = 'EOM_pulse'+str(i),
                link_start_to = 'end' )
...
# intial setup
m = LDEMeasurement('Test-ZPL-reflections-from-LT2-10ns-pulse-where-tail-is-higher plu threshold', 'LDE')
...
m.SP_duration               = 9000 #NOTE: changed this for second spin photon try
m.wait_after_SP             = 100 #in ns

# spin manipulation
m.MW_power_lt1              = 20 
m.MW_power_lt2              = 20 
m.MW_freq_lt1               = 2.826643e9 ###2.827197E9 #NOTE Needs to be adjusted
m.MW_freq_lt2               = 2.8283e9 #NOTE Adjust this by calibrating the pulses.
m.wait_after_pi2            = 0  #NOTE: change this for regular pulses!
...
m.eom_start                 = 40
m.eom_off_duration          = 100 #NOTE: change this to duration of CORPSE or regular!
...
m.wait_after_opt_pi         = 80 # determines start of MW pi pulse NOTE: change this for regular pulses!
m.wait_after_opt_pi2_lt1    = 164 # determines start of basis rotation NOTE: change this for regular pulses!
m.wait_after_opt_pi2_lt2    = 162 # determines start of basis rotation NOTE: change this for regular pulses!
...
m.pi2_lt1_amplitude          = 0.5094#0.5076#0.4934#0.4991#0.4818  #NOTE
m.pi2_lt2_amplitude          = 0.5977#0.6067  #NOTE #  only for regular pulse sequence

    # Pi pulse
m.pi_lt1_duration           = 50 #58
m.pi_lt2_duration           = 50
m.pi_lt1_amplitude          = 0.4495#0.4606#0.44785#0.4491#0.45295 #NOTE
m.pi_lt2_amplitude          = 0.5524#0.5745 #NOTE #  only for regular pulse sequence

# MW Pulses (CORPSE)

m.CORPSE_pulses             = False
m.lt1_CORPSE_pi             = False
m.CORPSE_single_pi_pulse_duration       = 58.
m.CORPSE_pi_over_two_single_pi_duration = 58.
m.time_between_CORPSE                   = 20.

m.CORPSE_pi_amp_lt1             = 0.5
m.CORPSE_pi_over_two_amp_lt1    = 0.5 
m.CORPSE_pi_amp_lt2             = 0.5113#NOTE
m.CORPSE_pi_over_two_amp_lt2    = 0.5372#NOTE
...
adpar['CR_preselect']               = 15 # is overwritten by the TPQI monitor
adpar['CR_probe']                   = 10 # is overwritten by the TPQI monitor
adpar['green_repump_duration']      = 10
adpar['wait_before_SSRO']           = 3
adpar['SSRO_duration']              = 27  #NOTE
adpar['SSRO_duration_lt1']          = 14  #NOTE 
...
adpar_lt1['CR_preselect']               = 45
adpar_lt1['CR_probe']                   = 35
