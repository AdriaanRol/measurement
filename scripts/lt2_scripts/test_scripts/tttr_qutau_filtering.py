...
class QuTau_TTTR():
    ...
    def check_for_decreasing_timestamps(self, tsdata):
        """
        Checks if there are decreasing timestamps in there.
        """
    ...
    def plot_histogram(self, data, channela, channelb):
        ...
        plt.figure()
        plt.hist(dts*binsize, color = 'red', alpha = 0.75, log = True)   
        plt.title('Histogram of channel B w.r.t. syncs')
        plt.xlabel('dt (ns)')
