...
def generate_sequence(do_program=True):
    ...
    seq.add_element('optical_rabi')#'optical_rabi',event_jump_target='idle')

        seq.add_pulse('start',  chan_hhsync, 'optical_rabi',         
                start = par_rabi_cycle_duration, duration = 50, 
                amplitude = 2.0, start_reference = last_start,  
                link_start_to = 'start') 
        last_start = 'start'

        seq.add_pulse('Gate_PLU_1',  chan_eom_aom, 'optical_rabi', 
                start = par_aom_start, duration = par_aom_duration, 
                start_reference = last_start, link_start_to = 'start')

        seq.add_pulse('start'+'delay',  chan_hhsync, 'optical_rabi', 
...
XXXXXXXXXXXX
    ...
    if not debug_mode:
        ...
        for i in arange(par_pre_rabi_syncs):
            seq.add_pulse('pre_start',  chan_hhsync, 'optical_rabi', 
                    start = -par_rabi_cycle_duration, duration = 50, 
                    start_reference = last,  link_start_to = 'start') 
            last = 'pre_start'
        ...
        for i in arange(par_post_rabi_syncs):
            seq.add_pulse('post_start',  chan_hhsync, 'optical_rabi', 
                    start = par_rabi_cycle_duration, duration = 50, 
                    start_reference = last,  link_start_to = 'start') 
            last = 'post_start'
...
def optimize():
    ...
    if (adwin_lt1.get_countrates()[par_opt_counter_lt1-1] < par_opt_min_cnts_lt1):
        ...
        ins_optimiz0r_lt1.optimize(cnt=par_opt_counter_lt1, cycles = 2, int_time=100)
    ins_green_aom_lt1.set_power(0)
    ...
    if (adwin_lt2.get_countrates()[par_opt_counter_lt2-1] < par_opt_min_cnts_lt2):
        ...
        ins_optimiz0r_lt2.optimize(cnt=par_opt_counter_lt2, cycles = 2, int_time=100)
    ins_green_aom_lt2.set_power(0)
