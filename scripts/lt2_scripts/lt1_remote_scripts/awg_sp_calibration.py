...
class AWGSPCalibration(PulsarMeasurement):
    ...
    def generate_sequence(self, upload=True):
        ...
        for i in range(self.params['pts']):
            e = element.Element('AWG_SP_sweep-%d' % i, pulsar=qt.pulsar_remote)
            e.add(TT, name='wait')
...
def awgspcalibration(name):
    ...
    m.params['AWG_SP_power'] = np.linspace(0,300e-9,m.params['pts'])
    m.params['AWG_SP_duration'] = 50e-6*np.ones(m.params['pts'])#np.linspace(0,60e-6,m.params['pts'])
