...
def plot_SSRO_data(datapath, noof_datapoints = 20, mw_min_freq = 2.865e9, mw_max_freq = 2.895e9, SSRO_duration = 30):
    ...
    add_ms0 = 0
    add_ms1 = 0
    ms0_vs_mwfreq = zeros(noof_datapoints)
    ms1_vs_mwfreq = zeros(noof_datapoints)
    ...
    for k in arange(noof_datapoints):
        ...
        ms0_vs_mwfreq[k] = add_ms0
        ms1_vs_mwfreq[k] = add_ms1
    ...
    plt.plot(mw_freq, ms0_vs_mwfreq/norm_factor, 'or')
    ...
    plt.plot(mw_freq, ms1_vs_mwfreq/norm_factor, 'or')    
    plt.xlabel('MW length (ns)')
    ...
    plt.plot(mw_freq,counts_during_readout, '-sr')
    ...
    plt.imshow(total_hist, cmap = 'hot', interpolation = None)
    ...
    g = load(datapath+'\\dark_esr-0_ChargeRO_before.npz')
    h = load(datapath+'\\dark_esr-0_ChargeRO_after.npz')
...
plot_SSRO_data(r'D:\measuring\data\20120612\195644_dark_esr_SIL9', 
        noof_datapoints = 40,
        mw_min_freq = 2.870e9,
        mw_max_freq = 2.885e9,
        SSRO_duration = 20)
