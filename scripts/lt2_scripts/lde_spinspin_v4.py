...
class LDEMeasurement(meas.Measurement):
    ...
    def generate_sequence(self, do_program=True):
        ...
        if self.lt1_CORPSE_pi:
            seq.add_pulse('pulse_420_lt1', channel = chan_mwI_lt1, element = 'lde',
             ...
             seq.add_pulse('pulse_mod_420_lt1', channel = chan_mw_pm, element = 'lde',
                duration = 2*self.MW_pulsemod_risetime_lt1,
                start = 0,start_reference = 'pi_pulse_lt1', link_start_to = 'start', 
                amplitude = self.MW_pulsemod_amplitude)
        ...
        if self.lt1_CORPSE_pi:
            seq.add_pulse('pulse_60_lt1', channel = chan_mwI_lt2, element = 'lde',
        ...
        if self.basis_rot:
            ...
            Iamplt2 = np.cos(np.pi*(self.basis_rot_angle+180.)/180.)*self.CORPSE_pi_over_two_amp_lt2
            Qamplt2 = np.sin(np.pi*(self.basis_rot_angle+180.)/180.)*self.CORPSE_pi_over_two_amp_lt2
        ...
        seq.add_pulse('pi2_pulse_I_lt1', channel = chan_mwI_lt1, element = 'lde',
                start = self.wait_after_opt_pi2+(self.pulse_384_length+self.pulse_318_length+self.pulse_24_length-self.pi2_lt1_duration)/2.,
                start_reference = last,link_start_to = 'end', 
                duration = self.pi2_lt1_duration, 
        ...
        seq.add_pulse('pi2_pulse_mod_lt1', channel = chan_mw_pm, element = 'lde',
                duration = 2*self.MW_pulsemod_risetime_lt1,
                start = 0,start_reference = 'pi2_pulse_I_lt1', link_start_to = 'start', 
                amplitude = PMamp)
        
        seq.add_pulse('pulse_384_I_lt2', channel = chan_mwI_lt2, element = 'lde',
                start = self.wait_after_opt_pi2, start_reference = last,link_start_to = 'end', 
                duration = self.pulse_384_length, 
    ...
    def check_suppression(self): #TODO: NEEDS TO BE TESTED
        adwin_lt1.set_simple_counting() 
        adwin_lt2.set_simple_counting()
        counters_lt2.set_is_running(True)
        counters_lt1.set_is_running(True)
...
# intial setup
m = LDEMeasurement('Spin-Photon-LT1-without-pi2', 'LDE')

### measurement parameters

# gate parameters
m.set_gate_good_phase       = -1 #either 1 or -1
m.set_phase_locking_on      = 0#1 #either 0 (off) or 1 (on)
...
m.Ex_RO_power_lt1           = 7e-9     # FIXME: get these from calibration measurement
m.A_RO_power_lt1            = 0
...
# general LDE
m.measurement_time          = 6*60 #in seconds
...
m.MW_power_lt1              = 20 
m.MW_power_lt2              = 20 
m.MW_freq_lt1               = 2.827197E9 #NOTE Needs to be adjusted
m.MW_freq_lt2               = 2.828378e9 #NOTE Adjust this by calibrating the pulses.
m.wait_after_pi2            = 0
...
m.pi2_lt1_amplitude          = 0.#0.4991#0.4818  #NOTE
m.pi2_lt2_amplitude          = 0.58   #NOTE  only for regular pulse sequence

    # Pi pulse
m.pi_lt1_duration           = 50 #58
m.pi_lt2_duration           = 50
m.pi_lt1_amplitude          = 0.#0.4491#0.45295 #NOTE
m.pi_lt2_amplitude          = 0.58   #NOTE  only for regular pulse sequence
...
if not(debug_mode):
    m.CORPSE_pi_amp_lt1             = 0.0
    m.CORPSE_pi_over_two_amp_lt1    = 0.0
    m.CORPSE_pi_amp_lt2             = 0.47825
    m.CORPSE_pi_over_two_amp_lt2    = 0.4941

    m.MW_pulsemod_amplitude         = 0.#2.
else:
...
adpar['SSRO_duration']              = 34   #FIXME get this from calibration meas
adpar['SSRO_duration_lt1']          = 14    #FIXME get this from calibration meas.
