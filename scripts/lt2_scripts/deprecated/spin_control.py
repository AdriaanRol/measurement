...
nr_of_datapoints = 100       #max nr_of_datapoints should be < 10000
repetitions_per_datapoint = 100000
min_mwpulse_length = 0    #in ns
max_mwpulse_length = 1000    #in ns
mwfrequency = 2.878         #in GHz
mwpower = 20                #in dBm
...
microwaves.set_frequency(mwfrequency*1E9)
...
par['green_readout_amplitude'] =      35e-6
...
def start_measurement():
    ...
    while (physical_adwin.Process_Status(9) == 1):
        ...
        pct = 100*reps_completed/par['SSRO_repetitions']

        if pct > pct_old:
            print('%s%% completed'%pct)
