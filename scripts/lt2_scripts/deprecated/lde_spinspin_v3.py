...
class LDEMeasurement(meas.Measurement):
    ...
    def generate_sequence(self, do_program=True):
        ...
        if self.lt1_CORPSE_pi:
            seq.add_pulse('pulse_420_lt1', channel = chan_mwI_lt1, element = 'lde',
             ...
             seq.add_pulse('pulse_mod_420_lt1', channel = chan_mw_pm, element = 'lde',
                ...
                start = 0,start_reference = 'pi_pulse_t1', link_start_to = 'start', 
                amplitude = self.MW_pulsemod_amplitude)
        ...
        seq.add_pulse('wait_I_1', channel = chan_mwI, element = 'lde',
                start = 0,start_reference = 'pulse_384_I_lt2',link_start_to ='end', 
                duration = self.time_between_CORPSE, amplitude = 0)
        seq.add_pulse('wait_Q_1', channel = chan_mwQ, element = 'lde',
        ...
        seq.add_pulse('wait_I_2', channel = chan_mwI, element = 'lde',
                start = 0,start_reference = 'pulse_318_I_lt2',link_start_to ='end', 
                duration = self.time_between_CORPSE, amplitude = 0)
        seq.add_pulse('wait_Q_2', channel = chan_mwQ, element = 'lde',
    ...
    def save(self, **kw):
        ...
        if not debug_mode: shutil.move(self.rawdir, os.path.join(self.save_folder, 'rawdata'))
        self.rawdir = ''
    ...
    def optimize(self, lt1 = False, lt2 = True): #TODO: NEEDS TO BE TESTED
        ...
        if lt1:
            ...
            qt.msleep(1)
            counts = adwin_lt1.get_countrates()[self.opt_counter_lt1-1]
            if counts < self.opt_threshold_lt1:
                print 'lt1 counts', counts,'Optimizing LT1...'
                green_aom_lt1.set_power(self.green_opt_power_lt1)
                optimizor_lt1.optimize(cnt = self.opt_counter_lt1, 
                        cycles = self.noof_opt_cycles)
            else:
                print '\tLT1 counts',counts,' high enough...'
            green_aom_lt1.set_power(0)

        if lt2:
            green_aom_lt2.set_power(self.green_opt_power_lt2)
            qt.msleep(1)
            counts = adwin_lt2.get_countrates()[self.opt_counter_lt2-1]
            if counts < self.opt_threshold_lt2:
                print 'lt2 counts',counts,'Optimizing LT2...'                
                green_aom_lt2.set_power(self.green_opt_power_lt2)
                optimizor_lt2.optimize(cnt = self.opt_counter_lt2, 
                        cycles = self.noof_opt_cycles)
            else: 
                print '\tLT2 counts',counts,' high enough...'
...
m.Ex_RO_power_lt1           = 9e-9     # FIXME: get these from calibration measurement
m.A_RO_power_lt1            = 0
...
m.MW_power_lt1              = 20 
m.MW_power_lt2              = 20 
m.MW_freq_lt1               = 2.82876E9 #NOTE Needs to be adjusted
...
if not(debug_mode):
    ...
    m.CORPSE_pi_amp_lt2             = 0.484
    m.CORPSE_pi_over_two_amp_lt2    = 0.50
...
adpar['SSRO_duration']              = 47   #FIXME get this from calibration meas
adpar['SSRO_duration_lt1']          = 17    #FIXME get this from calibration meas.
