...
class SSROMeasurement(Measurement):
    def __init__(self, name, **kw):
        ...
        self.par_cr_threshold = 50 # not used in preselect mode!
       
#        self.ins_setup = instruments['TheSetup']
        self.ins_awg = instruments['AWG']
        self.ins_pq = instruments['HH_400']      

        self.par_ADwin_CR_threshold    = 1 # will goto init if CR counts smaller
        self.par_ADwin_probe_duration  = 5
        self.par_ADwin_probe_threshold = 1 # will be happy to continue if larger
...
#        self.set_sequence_reset_event(pqm.EV_MA_1+pqm.EV_auto)
        ...
        self.add_ADwin_code('conditional repump', 
                ...
                [[45,1], [63,7],[73,self.par_ADwin_AOM_duration],[74,self.par_ADwin_probe_duration],
...
def ssro(name, reps=10000, do_ms0=True, do_ms1=True):
    ...
    debug_index += 1
    #print(debug_index)    
    m.par_ro_Ex_amplitude = exaom.power_to_voltage(9e-9)
    m.par_ro_A_amplitude = 0.
    ...
    #print(debug_index)    
    m.par_binsize = 17
    m.par_cr_threshold = 0 #20 # not used!
...
#    m.par_ro_Ex_amplitude = 0.77
    m.par_ro_Ex_amplitude = 0.77
    m.par_ro_A_amplitude = 0.
    aamplitude = aaom.power_to_voltage(50e-9) 
    ...
    m.par_cr_Ex_amplitude = exaom.power_to_voltage(25e-9)
    m.par_cr_A_amplitude = aamplitude

    for p in linspace(0, 15, 11)*1e-9:
        m.par_ro_Ex_power = p
...
#    m.par_ro_Ex_amplitude = 0.77
    m.par_ro_Ex_amplitude = instruments['MatisseAOM'].power_to_voltage(25e-9)
    m.par_ro_A_amplitude = 0.
    aamplitude = aaom.power_to_voltage(50e-9) 
    ...
    m.par_cr_Ex_amplitude = exaom.power_to_voltage(50e-9)
    m.par_cr_A_amplitude = aamplitude

    for p in linspace(0.5, 120, 2)*1e-9:
        m.par_ro_Ex_power = p
...
if __name__ == "__main__":
    ...
    #ssro_vs_examplitude('SIL10_lowpower')
