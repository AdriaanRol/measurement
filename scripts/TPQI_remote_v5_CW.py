...
time_limit = 2000 #in Adwin_proII processor cycles # in units of 1us
...
par_opt_min_cnts_lt1 = 0 #190000
par_opt_min_cnts_lt2 = 250000 #85000
...
par_sp_voltage_lt2 = .17

par_aom_start = -25 + 5000 #subtract aom rise time
par_aom_duration = 1900000 
par_eom_aom_amplitude = 0.21

# Parameters pulse gating unit #
use_apd_gate = 1                    #flag if the unit should be used
par_gate_start_offset = -1000          #shifts your counting window over the pulse
par_gate_duration = par_aom_duration   #determines the length of the counting window

# Measurement time
par_meas_time = 20 #measurement time in minutes
par_reps = 50
...
def measurement_cycle(hh_measurement_time):
    ...
    if hh_measurement_time > 0:
        ...
        while hharp.get_MeasRunning():
            qt.msleep(1)
        histogram = hharp.get_Histogram(0,clear=1)
