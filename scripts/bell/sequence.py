...
def pulse_defs_lt3(msmt):
    ...
    msmt.e_pulse = pulselib.MW_IQmod_pulse('MW pulse',
        ...
        PM_risetime = msmt.params_lt3['MW_pulse_mod_risetime'])
    ...
    msmt.CORPSE_pi = pulselib.IQ_CORPSE_pulse('CORPSE pi-pulse',
        ...
        PM_channel = 'MW_pulsemod',
        PM_risetime = msmt.params_lt3['MW_pulse_mod_risetime'],
        frequency = msmt.params_lt3['CORPSE_pi_mod_frq'],
        amplitude = msmt.params_lt3['CORPSE_amp'],
        rabi_frequency = msmt.params_lt3['CORPSE_rabi_frequency'],
    ...
    msmt.CORPSE_pi2 = pulselib.IQ_CORPSE_pulse('CORPSE pi2-pulse',
        ...
        PM_channel = 'MW_pulsemod',
        PM_risetime = msmt.params_lt3['MW_pulse_mod_risetime'],
        frequency = msmt.params_lt3['CORPSE_pi2_mod_frq'],
        amplitude = msmt.params_lt3['CORPSE_pi2_amp'],
        rabi_frequency = msmt.params_lt3['CORPSE_rabi_frequency'],
    ...
    msmt.plu_gate = pulse.SquarePulse(channel = 'plu_sync', amplitude = 1.0, 
                                    length = msmt.params_lt3['PLU_gate_duration'])
...
def _lt1_LDE_element(msmt):
    ...
    # this pulse to ensure that the element has equal length as the lt3 element
...
def _lt3_first_pi2(msmt, **kw):
    ...
    CORPSE_pi2_wait_length = msmt.params_lt3['CORPSE_pi2_wait_length'] #- (msmt.CORPSE_pi2.length - 2*msmt.params_lt3['MW_pulse_mod_risetime'])/2 
...
def _lt3_final_pi(msmt, name, time_offset, **kw):
    extra_t_before_pi = kw.pop('extra_t_before_pi', 0)
    CORPSE_pi_phase = kw.pop('CORPSE_pi_phase', 0)
    # around each pulse I make an element with length 1600e-9; 
    # the centre of the pulse is in the centre of the element.
    # this helps me to introduce the right waiting times, counting from centre of the pulses
    CORPSE_pi_wait_length = msmt.params_lt3['CORPSE_pi2_wait_length']  #- (msmt.CORPSE_pi2.length - 2*msmt.params_lt3['MW_pulse_mod_risetime'])/2 
