...
// changes: add channel-resolved detection
//       - new measure mode definition: link_to_previous_section = 8
//       - added field 'linked_sections' to section_type record
...
type
  ...
  section_type = record            // data record defining a measurement section
    ...
    linked_sections: integer;      // if following sections share the same
                                   // start (mode = link_to_previous_section),
                                   // this carries the count of the linked
                                   // sections, otherwise set to 0.
  end;
...
var
  ...
  measuring: boolean;       // True indicates that data processing is active;
...
begin
  writeln('');
  writeln('');
  writeln('universal measurementDLL v1.1');
  writeln('~~~~~~~~~~~~~~~~~~~');
  {$IFDEF HH400}
...
begin
  ...
  sections[l].linked_sections := 0;
  if (mode and link_to_previous_section) > 0 then
  begin
    m := l;

    while ((sections[m].mode and link_to_previous_section) > 0) and (m > 0) do
      m := m - 1;

    if (sections[m].mode and link_to_previous_section) > 0 then
      writeln('Error: first section cannot be linked to previous!')
    else
      sections[m].linked_sections += 1;
  end;
  begin
    ...
    begin
      ...
      writeln('##');
      if (sections[l].mode and link_to_previous_section) > 0 then
      begin
        writeln('## this section shares a common start with the previous section');
        writeln('##');
      end;
...
procedure do_initialize_section(); // prepare temporary data array for
                                   // conditional mode
                                   // initialize all following linked sections
var j,m:integer;
begin
  for m:=0 to sections[section_index].linked_sections do
  begin
    if (sections[section_index+m].mode and time_axis) > 0 then
    begin
      SetLength(sections[section_index+m].temp_data,sections[section_index+m].duration);
      for j:= 0 to sections[section_index+m].duration-1 do
        sections[section_index+m].temp_data[j]:=0
    end else begin
      SetLength(sections[section_index+m].temp_data, 1);
      sections[section_index+m].temp_data[0]:=0
    end;
    sections[section_index+m].counts:=0;
  end;
end;

procedure do_increment_section(); // handle loop behaviour for section
                                  // increment event
                                  // skip all following linked sections
begin
  // increment section
  incr_section := 0;
  time0 := time;
  ...
  begin
    ...
    sections_active := 1;
    section_index  := 0;
  end else if sections_active = 1 then
    section_index += (1 + sections[section_index].linked_sections);
...
var
  ...
  m: integer;                  // linked sections counter
    ...
    begin
      ...
      {$ENDIF}
        ...
        begin
          ...
          for m := 0 to sections[section_index].linked_sections do
            if (event and sections[section_index+m].reset_mode) > 0 then
              do_initialize_section();
          ...
          for m := 0 to sections[section_index].linked_sections do
          begin
            if ((event and sections[section_index+m].event_type) > 0) and
               (sections_active=1) and (repetitions_active=1) and
               (sweeps_active=1) and (measuring = True) then
            begin
              time1 := time + overflow;
              dt := time1 - time0;
              bin_index := dt div (1 shl sections[section_index+m].bin_size) -
                sections[section_index+m].offset;
              if (bin_index >= 0) and
                (bin_index < sections[section_index+m].duration) then
              begin
                // evaluate count event
                if (sections[section_index+m].mode and time_axis) > 0 then
                  sections[section_index+m].temp_data[bin_index]+=1;
                sections[section_index+m].counts+=1;
              end;
              if bin_index >= sections[section_index+m].duration then
                incr_section := incr_section or auto;
            end;
          end;
...
//    writeln('              ', ((valid_counter * 10000) div (valid_counter + invalid_counter))/100,'% valid events');
  ...
  if (meastime / 1000) > 0 then
    writeln('Average data rate: ', round((ma1_cnt + ma2_cnt + ma3_cnt + ma4_cnt + start_cnt + stop_cnt + sync_cnt + ovl_cnt) / (meastime)), ' events/s');
...
var
  ...
  m: integer;                  // linked sections loop variable
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                detected_events += 1;
                case (sections[section_index+m].mode and
                     (time_axis + repetition_axis + sweep_axis)) of
                   time_axis:
                         begin
                           sections[section_index+m].data[bin_index] += 1;
                         end;
                   sweep_axis:
                         begin
                           sections[section_index+m].data[sweep_index] += 1;
                         end;
                   repetition_axis:
                         begin
                           sections[section_index+m].data[repetition_index] += 1;
                         end;
                   time_axis + sweep_axis:
                         begin
                           sections[section_index+m].data[bin_index +
                             sections[section_index+m].duration*sweep_index] += 1;
                         end;
                   sweep_axis + repetition_axis:
                         begin
                           sections[section_index+m].data[sweep_index+sweeps*repetition_index] += 1;
                         end;
                   time_axis + repetition_axis:
                         begin
                           sections[section_index+m].data[bin_index+sections[section_index+m].duration*repetition_index] += 1;
                         end;
                   time_axis + repetition_axis + sweep_axis:
                         begin
                           sections[section_index+m].data[bin_index+sections[section_index+m].duration*sweep_index +
                                  sections[section_index+m].duration*sweeps*repetition_index] += 1;
                         end
                end;
              end;
