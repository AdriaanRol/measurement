...
'DIM DATA_27[max_repump_duration] AS FLOAT AT EM_LOCAL 'CR vs gate counts
'DIM DATA_28[max_CR_duration] AS FLOAT AT EM_LOCAL 'CR vs gate counts

DIM counter_channel, counter_pattern AS LONG
DIM repump_laser_DAC_channel, Ex_laser_DAC_channel, A_laser_DAC_channel AS LONG
DIM gate_DAC_channel, repump_freq_AOM_DAC_channel AS LONG
...
DIM repump_duration, CR_duration, SP_duration, SSRO_duration AS LONG
DIM SSRO_repetitions AS LONG
DIM SP_filter_duration, wait_after_pulse_duration AS LONG
...
DIM Ex_CR_voltage, A_CR_voltage, Ex_SP_voltage, A_SP_voltage, Ex_RO_voltage, A_RO_voltage, Ex_off_voltage, A_off_voltage AS FLOAT
DIM gate_control_offset, gate_control_amp, gate_control AS FLOAT
DIM repump_freq_control,repump_freq_control_offset, repump_freq_control_amp AS FLOAT

DIM timer, mode, i, cr_probe_timer, aux_timer AS LONG
...
INIT:
  ...
  gate_DAC_channel             = 1' DATA_20[24]
  repump_freq_AOM_DAC_channel  = 4' DATA_20[25]
  ...
  gate_control_offset          = 2.3401485321983473'= DATA_21[11]
  gate_control_amp             = 0.0005'= DATA_21[12]
  repump_freq_control_offset   = 5.0'= DATA_21[13]
  repump_freq_control_amp      = 0.0'= DATA_21[14]
  
  FPar_79 = gate_control_amp 'XXX
  FPar_80 = gate_control_offset
  ...
  FOR i = 1 TO max_stat
    DATA_26[i] = 0
  NEXT i
  'fill data_19 with a sine for gate control (calculating sines is slow --> cannot do it during the event cycle)
  FOR i = 1 TO CR_duration
    DATA_19[i] = Sin(-3.14+2*3.14*i/CR_duration)
    'DATA_28[i] = 0.0
  NEXT i
  'fill data_18 with a sine for repump freq control (calculating sines is slow --> cannot do it during the event cycle)
    ...
    SELECTCASE mode
      ...
      CASE 0    ' green repump
        ...
        ELSE 
          ...
          ELSE
            ...
            counts = CNT_READ(counter_channel)
            repump_freq_control=repump_freq_control_amp*DATA_18[timer]
            DAC(repump_freq_AOM_DAC_channel, 3277*(repump_freq_control+repump_freq_control_offset)+32768) ' put current voltage on freq mod aom
            'DATA_27[timer]=DATA_28[timer]*0.8+(counts-old_counts)
            FPar_78 = FPar_78*0.9999+repump_freq_control*(counts-old_counts)
            old_counts=counts
      ...
      CASE 1    ' Ex/A laser CR check
        ...
        IF (timer = 0) THEN
          ...
          old_counts = 0
          FPar_79 = -1.0*FPar_79
          'gate_control_amp = -1.0*gate_control_amp
          gate_control_amp = FPar_79
          gate_control_offset  = FPar_80
        ELSE 
          ...
          ELSE
            counts = CNT_READ(counter_channel)
            gate_control=DATA_19[timer]
            IF (Par_16 > 0) THEN
              DAC(gate_DAC_channel, 3277*(gate_control_amp*gate_control+gate_control_offset)+32768) ' put current voltage on gate
            ENDIF
            
            'DATA_28[timer]=DATA_27[timer]*0.8+(counts-old_counts)
            FPar_77 = FPar_77*0.99999+gate_control*(counts-old_counts)
            old_counts=counts
          ENDIF
        ENDIF
