...
DIM i as long
DIM cr_prepared as integer              ' set to 1 if both NVs are prepared, reset to 0 after first CR.
...
INIT:
  ...
  P2_DAC(DAC_MODULE, green_aom_channel, 32768)
...
EVENT:
  ...
  if (do_repump > 0) then
    ...
    if (current_repump_step > repump_steps) then
      ...
      P2_DAC(DAC_MODULE, green_aom_channel, 32768) ' turn off green
