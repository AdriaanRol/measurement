...
DIM green_aom_voltage AS LONG           ' voltage of repump pulse
DIM green_aom_voltage_zero AS LONG      ' voltage to get minimum GreenAOM leakage 
dim ex_aom_channel as long
dim ex_aom_voltage as long
dim a_aom_channel as long
dim a_aom_voltage as long
...
INIT:
  ...
  green_aom_voltage_zero = fpar_33
  ex_aom_channel = par_30
  ex_aom_voltage = fpar_31
  a_aom_channel = par_31
  a_aom_voltage = fpar_32
  
  trigger_dio_in = 8 'par_32 later maybe as a variable
  trigger_dio_out = 16 'par_33 later maybe as a variable
  
  P2_DAC(DAC_MODULE, green_aom_channel, 3277*green_aom_voltage_zero+32768)
...
EVENT:
  ...
  if (do_cr_check > 0) then
    ...
    if (do_cr_lt2_check > 0) then
      ...
      if (current_cr_check_step > cr_check_steps) then
        ...
        if (counter = 5) then
          current_cr_check_counts = P2_CNT_READ_LATCH(CTR_MODULE, 1) + P2_CNT_READ_LATCH(CTR_MODULE, 2)
        else
    ...
    if ((is_cr_lt1_check_running=1) and (do_cr_lt1_check=1)) then
      ...
      is_cr_lt1_OK = ((P2_DIGIN_LONG(DIO_MODULE)) AND (2^trigger_dio_in)) ' check whether LT1 is OK (trigger by ttl)
  ...
  if (do_repump > 0) then
    if (current_repump_step = 0) then
      ...
      P2_DAC(DAC_MODULE, green_aom_channel, 3277*green_aom_voltage+32768) 'turn on green
    ...
    if (current_repump_step > repump_steps) then
      ...
      P2_DAC(DAC_MODULE, green_aom_channel, 3277*green_aom_voltage_zero+32768) ' turn off green
