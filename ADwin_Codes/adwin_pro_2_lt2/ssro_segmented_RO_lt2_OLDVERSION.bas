...
'<Header End>
' this program implements a conditional SSRO, which stops as soon as a click is detected.
' Then we perform a second (full) SSRO to detect the amount fo spin flips.
...
DIM DATA_27[max_SP_bins] AS LONG AT EM_LOCAL  ' segment number of the detected click (0=no click within read-out time)
...
INIT:
  ...
  segmented_RO_duration        = DATA_20[20]
  ...
  segmented_Ex_RO_voltage      = DATA_21[9]
  ...
  FOR i = 1 TO max_stat
    DATA_26[i] = 0
  NEXT i
  FOR i = 1 TO segmented_RO_duration
    DATA_27[i]=0
  NEXT i  
