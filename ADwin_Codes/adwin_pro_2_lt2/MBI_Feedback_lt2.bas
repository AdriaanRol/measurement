...
DIM seg_timerone AS LONG
DIM seg_timertwo AS LONG
    ...
    SELECTCASE mode
      ...
      CASE 7    ' spin readout
        ...
        ELSE 
          ...
          RO_click = P2_CNT_READ(CTR_MODULE, counter_channel)
          IF ((timer = RO_duration) OR (RO_click > 0)) THEN
            IF ((RO_cntr=0) AND (seg_timerone = 0)) THEN
              seg_timerone=timer
            ENDIF
            IF ((seg_timertwo=0) AND (RO_cntr=1)) THEN
              seg_timertwo=timer
            ENDIF
          ...
          IF (timer = RO_duration) THEN
            ...
            PAR_60 = PAR_60 + RO_click'P2_CNT_READ(CTR_MODULE, counter_channel) 'total spin-RO counts
            
            'counts = P2_CNT_READ(CTR_MODULE, counter_channel) - old_counts
            counts=RO_click-old_counts
            old_counts = counts
            ...
            IF (RO_cntr=1) THEN
              IF(final_RO=1)THEN
                IF (RO_click > 0) THEN
                  ...
                  'DATA_35[(seg_timerone*sweep_length)+sweep_index+1] = DATA_35[(seg_timerone*sweep_length)+sweep_index+1] + 1
                ENDIF                
              ELSE
                IF (RO_click > 0) THEN
