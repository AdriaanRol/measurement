...
INIT:
  ...
  AOM_channel = PAR_26     '4 'P26
  AOM_voltage = FPAR_30    '7.0 'FP30
  AOM_duration = PAR_27    '5 'P27
  probe_duration = PAR_28  '10 'P28
...
EVENT:
  ...
  if ((P2_DIGIN_EDGE(DIO_MODULE, 0) AND 2^9) > 0) then
    ...
    PAR_41 = counts ' P51
    index = index + 1
    DATA_51[index] = counts 'D51
    if (index = 1000) then index = 0
    PAR_52 = 0 'P52
    for i = 1 to 1000
      PAR_52 = PAR_52 + DATA_51[i] 'P52, D51
    next i
  endif
